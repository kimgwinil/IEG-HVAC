...
function EnergyScreen({ s, L }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateRows: '88px 112px minmax(0, 1fr)', gap: 6, height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.35fr 1fr', gap: 8, minHeight: 0 }}>
        ...
        <div className="card" style={{ display: 'grid', gridTemplateRows: 'auto auto 1fr', minHeight: 0, padding: 6 }}>
          <div className="card-h"><div className="title">Energy Breakdown</div></div>
          <StackedBar segments={breakdown} height={10} />
          <div style={{ display: 'grid', gap: 2, marginTop: 4, alignContent: 'start', overflow: 'auto' }}>
            {breakdown.map(b => (
              <div key={b.label} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10 }}>
                ...
                <span style={{ fontFamily: 'JetBrains Mono', color: 'var(--ink)', fontWeight: 600, fontSize: 9.5 }}>{b.value.toFixed(1)}</span>
                <span style={{ color: 'var(--ink-4)', fontSize: 9, width: 26, textAlign: 'right', fontFamily: 'JetBrains Mono' }}>
